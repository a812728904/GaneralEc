...
/**
 * Author:SharedPreferencesHelper
 * Created by LMX on 2018/4/16
 * Description:
 */
@SuppressLint("NewApi")
public class SharedPreferencesHelper  {
    private volatile static SharedPreferencesHelper util;
    private static SharedPreferences sp;
    private final static String SHARENAME="share";
    private SharedPreferencesHelper() {
        sp = General.getApplicationContext().getSharedPreferences(SHARENAME, Context.MODE_PRIVATE);
    }
    /**
    ...
    public static SharedPreferencesHelper getInstance() {
        if (util == null) {
            synchronized (CookiesIstration.class) {
                if (util == null) {
                    util = new SharedPreferencesHelper();
                }
            }
        }
        return util;
